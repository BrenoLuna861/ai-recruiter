<template>
  <div class="chat-page">
    <div class="page-header">
      <h1>Agente <em>Alex</em></h1>
      <p class="subtitle">Seu headhunter virtual · Análise · Match · Orientação</p>
    </div>
    ...
    <div class="chat-container card">
      ...
      <div class="messages" ref="messagesEl">
        <div class="welcome" v-if="!messages.length">
          ...
          <div class="welcome-title">Olá, eu sou o Alex</div>
          <div class="welcome-body">Seu agente de IA especialista em recrutamento. Pergunte sobre seu currículo, vagas, entrevistas ou desenvolvimento de carreira.</div>
...
.page-header { margin-bottom: 20px; padding-bottom: 20px; border-bottom: 1px solid var(--border); }
...
.welcome-icon { font-size: 2.5rem; color: var(--accent); margin-bottom: 16px; }
.welcome-title { font-family: var(--font-display); font-size: var(--text-2xl); letter-spacing: -0.02em; margin-bottom: 8px; }
.welcome-body { font-size: var(--text-sm); color: var(--text-muted); line-height: 1.7; margin-bottom: 24px; }
.suggestions { display: flex; flex-direction: column; gap: 8px; }
.suggestion {
  background: var(--bg-3); border: 1px solid var(--border);
  color: var(--text-muted); font-family: var(--font-body);
  font-size: var(--text-sm); padding: 10px 16px;
...
.msg-text { font-size: var(--text-sm); line-height: 1.7; }
...
.send-btn {
  ...
  display: flex; align-items: center; justify-content: center;
  font-size: 16px; font-weight: 700; color: #0a0a0b;
  flex-shrink: 0; transition: all 0.15s;
}
.send-btn:hover:not(:disabled) { background: var(--accent-hover); transform: scale(1.05); }
.send-btn:disabled { opacity: 0.4; cursor: not-allowed; }

.chat-hint { font-size: 10px; color: var(--text-faint); text-align: center; padding-bottom: 8px; letter-spacing: 0.05em; }
</style>
